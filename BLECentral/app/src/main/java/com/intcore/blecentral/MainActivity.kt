package com.intcore.blecentral
...
import com.intcore.blecentral.R
...
class MainActivity : AppCompatActivity() {
    ...
    var powerLookupTable = HashMap<String, Int>()
...
//        if (!characteristic.isWriteableWithoutResponse()) {
//            appendLog("ERROR: write failed, characteristic not writeable without response $CHAR_FOR_WRITE_UUID")
////            refreshScanning()
//            return
//        }
    ...
    private fun prepareAndStartBleScan() {
        ensureBluetoothCanBeUsed { isSuccess, message ->
            appendLog("prepareAndStartBleScan= $message - isSuccess= $isSuccess")
            if (isSuccess) {
                safeStartBleScan()
            }
        }
    ...
    private val scanSettings: ScanSettings
        get() {
            return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                appendLog("Scan Setting scanSettingsSinceM")
                scanSettingsSinceM
            } else {
                appendLog("Scan Setting scanSettingsBeforeM")
                scanSettingsBeforeM
            }
        }
    ...
    private val scanCallback = object : ScanCallback() {
        override fun onScanResult(callbackType: Int, result: ScanResult) {
            ...
            val distance: Double = calculateAccuracy(
                result.rssi,
            )

            appendLog(
                "onScanResult name=$name address= ${result.device?.address} rssi=${result.rssi} distance= $distance"
            )


//            if (distance in 0.0032 .. 0.3017) { // Range between -35 and -55
...
//                override fun run() {
//                    if (sortedList.last() in -40 downTo -60) {
//                        lifecycleState = BLELifecycleState.Connecting
//                        result.device.connectGatt(this@MainActivity, false, gattCallback)
//                        safeStopBleScan()
//                    }
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        ...
        override fun onReadRemoteRssi(gatt: BluetoothGatt?, rssi: Int, status: Int) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                Log.d(
                    "BluetoothRssi",
                    String.format("BluetoothGat ReadRssi[%d]", rssi)
                )
            }
        }
        ...
        override fun onCharacteristicRead(
            ...
            if (characteristic.uuid == UUID.fromString(CHAR_FOR_READ_UUID) && characteristic.value != null) {
...
//                        if (lifecycleState == BLELifecycleState.Connected) {
//                            refreshScanning()
//                        }
                        "error $status"
                    }
...
//    }
    ...
    private fun calculateAccuracy(rssi: Int): Double {
        ...
        val txPower = -62 // Static average value for transmit power
